...
def lambda_handler(event, lambda_context):
    ...
    if named:
        ...
        if spot_request_id:
            ...
            message = {'Instance':instance_id, 'Spot_Fleet':spot_request_id}
            sqs.send_message(QueueUrl=queue_url, MessageBody=message)

            queue_messages = sqs.receive_message(QueueUrl=queue_url, MaxNumberOfMessages = 10)
            fleetlist = []
            for msg in queue_messages['Messages']:
                # List nameless spot fleet machines killed in last hour
                if time.time() - msg['Attributes']['SentTimestamp'] < 60*60:
                    fleet = msg['Body']['Spot_Fleet']
                    fleetlist.append(fleet)
                # Clean up while we're at it: Delete messages that are over a day old
                if time.time() - msg['Attributes']['SentTimestamp'] > 60*60*24:
            ...
            if countdict[spot_request_id] > 2:
                activeinstances = ec2.describe_spot_fleet_instances(SpotFleetRequestId=spot_request_id)
            ...
            if not has_named_instance:
                print (f"{spot_request_id} has no named instances. Cancelling spot request.")
                ec2.cancel_spot_fleet_requests(SpotFleetRequestIds=[spot_request_id], TerminateInstances=True)
